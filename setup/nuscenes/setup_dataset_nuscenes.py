...
import utils.src.net_utils as net_utils
...
python setup/setup_dataset_nuscenes.py --nuscenes_data_root_dirpath data/nuscenes --nuscenes_data_derived_dirpath data/nuscenes_derived --n_thread 32
'''
...
parser.add_argument('--n_forward_frames_to_reproject',
    type=int, default=15, help='Number of forward frames to project onto a target frame')
parser.add_argument('--n_backward_frames_to_reproject',
    type=int, default=15, help='Number of backward frames to project onto a target frame')
parser.add_argument('--paths_only',
    action='store_true', help='If set, then only produce paths')
parser.add_argument('--n_thread',
    type=int, default=30, help='Number of threads to use in parallel pool')
...
outlier_removal = net_utils.OutlierRemoval(
    ...
    while sample_token != last_sample_token:
        ...
        if not paths_only:
            ...
            if args.filter_threshold_photometric_reconstruction > 0:
                ...
                points = net_utils.backproject_to_camera(ground_truth, camera_intrinsics, shape)
                ...
                if prev_sample_token != '':
                    ...
                    prev_camera_image = \
                        torch.from_numpy(net_utils.load_image(prev_camera_image_path, data_format='CHW')).unsqueeze(0)
                    prev_camera_token = prev_sample['data']['CAM_FRONT']
                    ...
                    target_xy_prev = net_utils.project_to_pixel(
                        points=points,
                        pose=current_to_prev_pose,
                        intrinsics=camera_intrinsics,
                        shape=shape)

                    reprojected_image_from_prev = net_utils.grid_sample(
                ...
                if next_sample_token != '':
                    ...
                    target_xy_next = net_utils.project_to_pixel(
                        points=points,
                        pose=current_to_next_pose,
                        intrinsics=camera_intrinsics,
                        shape=shape)

                    reprojected_image_from_next = net_utils.grid_sample(
